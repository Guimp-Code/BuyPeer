        ...
        login: function () {
            try {
                ...
                this.$store.dispatch('login', this.form).then((res) => {
                    ...
                    // Se o usuário tiver menu/permissões de admin, redireciona para o painel
                    const defaultMenu = this.$store.getters.authDefaultMenu;
                    if (defaultMenu && Object.keys(defaultMenu).length > 0 && defaultMenu.url) {
                        router.push({ path: `/admin/${defaultMenu.url}` });
